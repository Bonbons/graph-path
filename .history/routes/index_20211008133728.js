...
const fetchMachine = (session) => createMachine({
  id: 'login',
  initial: 'login',
  context: {
    session: session,
        ...
        LOGIN: {
          ...
          actions: assign({
            session: (context, event) => {
              console.log('session', session.bar);
              console.log('************************************');
              return session;
            }
          })
...
function sessionCheck(req, res, next) {
  if(!req.session.machineService){
    if (!has(machineServices, req.session.id)) {
      machineServices[req.session.id]  = interpret(fetchMachine(newTest(req.session.id))).onStop(() => {
